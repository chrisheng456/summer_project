...
class AgendaSegmenterPipeline:
    def process(self, info: ProcessInformation):
        ...
        if not detail or not lines:
            logger.warning("AgendaSegmenter: Missing meeting detail or transcription, skipping")
            return
        ...
        if not meeting_start or not agenda:
            logger.warning("AgendaSegmenter: Missing meeting startTime or agenda, skipping")
            return
        ...
        starts: List[datetime] = [None] * len(agenda)
