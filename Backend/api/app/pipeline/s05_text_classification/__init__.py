...
# Batch sizes (tunable depending on GPU memory)
...
class TextClassificationPipeline:
    """
    A pipeline for agenda text classification and short explanations.

    Workflow:
    - Split text into chunks within token limits
    - Run zero-shot classification in batches and aggregate scores
    - For certain categories, generate a one-sentence explanation
    - Reuse pipeline instances to avoid expensive reinitialization
    """
    ...
    def __init__(self) -> None:
        ...
        # Summarizer and tokenizer (used to create one-sentence explanations)
    ...
    def _aggregate_zeroshot(self, results) -> Dict[str, float]:
        """
        Combine classification results across multiple chunks
        by averaging the scores for each label.
        """
    ...
    def process(self, info: ProcessInformation):
        ...
        for item in agenda:
            ...
            # Classification: split into chunks → batch zero-shot → aggregate scores
            ...
            # Explanation: for key categories, generate one-sentence explanation
